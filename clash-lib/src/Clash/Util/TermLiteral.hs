...
import qualified Clash.Verification.Types        as Cv

import           Clash.Util.TermLiteralTH
...
instance TermLiteral Cv.CvRenderAs where
  termToData = $(deriveTermToData ''Cv.CvRenderAs)

instance TermLiteral a => TermLiteral (Cv.CvProperty' a) where
  termToData = $(deriveTermToData ''Cv.CvProperty')

instance TermLiteral a => TermLiteral (Cv.CvExpression' a) where
  termToData = $(deriveTermToData ''Cv.CvExpression')
